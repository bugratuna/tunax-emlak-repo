...
import { ListingJobData, score } from '../scoring/deterministic-scorer';

@Injectable()
export class ListingSubmittedProcessor implements OnModuleInit {
  private readonly logger = new Logger(ListingSubmittedProcessor.name);
  ...
  onModuleInit() {
    ...
    const worker = new Worker(
      ...
      async (job) => {
        ...
        const data: ListingJobData = job.data;
        ...
        this.logger.log(
          `Processing LISTING_SUBMITTED job ${job.id} for listing ${listingId}`,
        );

        const deterministicScores = score(data);

        this.logger.log(
          `Scores computed for listing ${listingId}: ` +
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Score endpoint returned ${response.status} for listing ${listingId}`,
          );
        }

        this.logger.log(
          `ScoringReport stored for listing ${listingId} (HTTP ${response.status})`,
        );

        // Module 3: initialize HITL enrichment scaffold (llmPrompt + llmJsonSchema)
        ...
        if (!enrichResponse.ok) {
          ...
          throw new Error(
            `Enrich endpoint returned ${enrichResponse.status} for listing ${listingId}`,
          );
        }

        this.logger.log(
          `Enrichment scaffold created for listing ${listingId} (HTTP ${enrichResponse.status})`,
        );
    ...
    worker.on('failed', (job, err) => {
      this.logger.error(JSON.stringify({
        ...
        maxAttempts: (job?.opts?.attempts as number | undefined) ?? 3,
    ...
    this.logger.log(
      'ListingSubmittedProcessor ready — listening on queue: automation',
    );
  }
}
